...
def generate_real_data_report(market_df, trade_risk, portfolio_metrics, stress_results, output_format="html"):
    ...
    for _, row in trade_risk.iterrows():
        ...
        ml_predictions[symbol] = {
            ...
            "VaR": row['VaR_95'],
            "ES": row['ES_95'],
            "PredictedVolatility": row['VaR_95'] / row['Value'] / 1.645
        }
    
    ml_predictions["Portfolio"] = {
        "Symbol": "Portfolio",
        "Value": portfolio_metrics['TotalValue'],
        "VaR": portfolio_metrics['VaR_95'],
        "ES": portfolio_metrics['ES_95'],
        "PredictedVolatility": portfolio_metrics['VaR_95'] / portfolio_metrics['TotalValue'] / 1.645
    }
...
def run_real_data_test(symbols=None):
    if symbols is None:
        symbols = ['AAPL', 'MSFT', 'AMZN', 'GOOGL', 'META']
    
    print(f"\n===== Running AI-Driven Trade Risk Assessment with Real Data =====\n")
    ...
    report_path = generate_real_data_report(market_df, trade_risk, portfolio_metrics, stress_results)
    
    print("\n===== Real Data Test Completed Successfully =====\n")
    print(f"Full risk report available at: {report_path}")
    
    return market_df, trade_df, trade_risk, portfolio_metrics, stress_results, report_path
